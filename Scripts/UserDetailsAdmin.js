$(document).ready(function () {
    ...
    form.find('.edit-button').on('click', function () {
        ...
        form.find('.datepicker .input-group-addon').removeClass("disabledbutton");
    ...
    function startDetailSave(modal) {
        ...
        var userDetails = { Id: ContextData.UserId };
        for (var i = 0; i < serializedForm.length; i++) {
            userDetails[serializedForm[i]['name']] = serializedForm[i]['value'];
        }

        userDetails['DOB'] = moment(userDetails['DOB'] + "Z", 'DD/MM/YYYYZ').toISOString();
        ...
        $.ajax({
            ...
            if (data.success) {
                currentDetails = form.serializeArray();
                ShowModal(smallModal, "Success", 'User details have been updated.');
    ...
    function closeEditor() {
        ...
        form.find('.datepicker .input-group-addon').addClass("disabledbutton");
        //form.find('[data-property]').show();
...
function removeAttachment(id) {
    attachmentId = id;
    ShowModal(largeModal, "Confirm", 'Are you sure you want to unassign this coach from the user?', 'Yes', DeleteCoachRow);
}

function DeleteCoachRow(modal) {
    ShowLoader(modal);

    $.ajax({
        method: "POST",
        url: "/User/DeleteCoachAttachment",
