$(document).ready(function () {
    ...
    function startDetailSave(modal) {
        ...
        $.ajax({
            ...
            if (data.success) {
                currentDetails = form.serializeArray();
                ShowModal(smallModal, "Success", 'Coach details have been updated.');
    ...
    $('#coach-email-send').on('click', function () {
        var coachid = $('#Id').val();
        var message = $('.coach-email-area').val();
        var subject = $('#coach-email-subject').val();
        $.ajax({
            method: "POST",
            url: "/Coach/SendEmail",
            data: { coachid: coachid, message: message, subject: subject }
        }).done(function (data) {
            $('#coach-email-dialog').modal('hide');
            $('.coach-email-area').trumbowyg('html', "");
            if (data.success) {
    ...
    $('#coach-email-back').on('click', function () {
        $('#coach-email-dialog').modal('hide');
        $('.coach-email-area').trumbowyg('html', "");
    });
...
function EmailPopup() {
    $('#coach-email-dialog').modal('show');
}

var attachmentId;
function removeAttachment(id) {
    attachmentId = id;
    ShowModal(largeModal, "Confirm", 'Are you sure you want to unassign this user from the coach?', 'Yes', DeleteUserRow);
}
